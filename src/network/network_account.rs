    ...
    where S: NetworkAccountStorage
{
    ...
    pub fn vp(world_state: &mut S) -> ValidatorPool<S, N, M> {
        ValidatorPool::new(
            world_state, 
            network_account_data::VALIDATOR_POOLS.to_vec()
        )
    }
    ...
    pub fn pools(world_state: &mut S, operator: PublicAddress) -> PoolDict<S, M> {
        ...
        PoolDict { prefix_key, world_state }
    }
    
    pub fn deposits(world_state: &mut S, operator: PublicAddress, owner: PublicAddress) -> DepositDict<S> {
        let prefix_key = [network_account_data::DEPOSITS.as_slice(), &operator, &owner].concat();
        DepositDict { prefix_key, world_state }
    }
    ...
    pub fn set_current_epoch(&mut self, current_epoch: u64) {
        self.world_state.set(&network_account_data::CURRENT_EPOCH, current_epoch.to_le_bytes().to_vec());
    }
}
