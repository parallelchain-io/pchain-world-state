...
/// IndexHeap supports the operations below in addition to [IndexMap].
    ...
    V: Clone + PartialEq + Eq + PartialOrd + Ord + Into<Vec<u8>> + From<Vec<u8>> + KeySpaced,
{
    ...
    pub fn insert_extract(&mut self, value: V) -> Result<Option<V>, IndexHeapOperationError> {
        ...
        let replaced = if length == self.capacity {
            if value < first_v {
    ...
    fn replace(&mut self, to_index: u32, to_v: V, from_index: u32, from_v: V) {
        ...
        let key_iv = [
            ...
            &IndexMap::<'a, T, V>::PREFIX_INDEX_VALUE,
            from_index.to_le_bytes().as_ref(),
        ]
        .concat();
        ...
        let key_ki = [
            self.domain.as_slice(),
            &IndexMap::<'a, T, V>::PREFIX_KEY_INDEX,
            to_v.key(),
        ]
        .concat();
