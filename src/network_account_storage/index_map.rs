...
use super::network_account::{KeySpaced, NetworkAccountStorage};
    ...
    V: Clone + PartialEq + Eq + Into<Vec<u8>> + From<Vec<u8>> + KeySpaced,
{
    ...
    pub(in crate::network_account_storage) fn index_of_key(&mut self, key: &[u8]) -> Option<u32> {
        ...
        self.store
            .get(&key_ki)
            .map(|index_bytes| u32::from_le_bytes(index_bytes.try_into().unwrap()))
    }
    ...
    pub fn get(&mut self, index: u32) -> Option<V> {
        ...
        self.store
            .get(&key_iv)
            .map(|value_types| value_types.into())
    }
...
fn test_index_map() {
    ...
    impl From<Vec<u8>> for TestString {
        fn from(bytes: Vec<u8>) -> Self {
            ...
            Self {
                name: String::from_utf8(r).unwrap(),
            }
        }
    }
