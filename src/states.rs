    ...
    where S: WorldStateStorage + Send + Sync + Clone 
{
    ...
    pub fn set_storage_value(&mut self, address: PublicAddress, key: AppKey, value: Value){
        ...
        self.inner.trie.set_value(WSKey::for_protected_account_data(&address, protected_account_data::STORAGE_HASH), new_storage_hash.to_vec());
    ...
    where S: WorldStateStorage + Send + Sync + Clone 
{
    ...
    pub fn cbi_version(&self, address: PublicAddress) -> (Proof, Option<u32>){
        ...
        let proof: Proof = proof.into_iter().map(|node| WSProofNode::new(proof_level::WORLDSTATE, node).into()).collect();
        (proof, cbi_version.map(|bytes| u32::from_le_bytes(bytes.try_into().unwrap())))
    }
    ...
    fn account_storage_hash(&self, address: &PublicAddress) -> (Proof, Option<Sha256Hash>){
        ...
        match storage_hash {
            Some(hash) => {
                (proof, Some(hash.try_into().unwrap()))
            },
            None => (vec![], None)
        }
    }
...
impl<S: WorldStateStorage + Send + Sync + Clone> WorldState<S>{
    ...
    /// Get the whole list of ket-value pair associated with this address
    /// Return None if
    ...
    pub fn commit(&mut self) {
        ...
        for address in addresses {
            ...
            let storage_hash_key = WSKey::for_protected_account_data(&address, protected_account_data::STORAGE_HASH);
            self.cached_changes.insert_world_state(storage_hash_key, new_storage_hash.to_vec());
        }
    ...
    pub fn commit_and_close(mut self) -> WorldStateChanges {
        ...
        WorldStateChanges{ inserts, deletes, next_state_hash }
    }
