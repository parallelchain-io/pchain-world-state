...
pub(crate) enum TrieLevel {
    ...
    WorldState,
    /// The level descended from [TrieLevel::WorldState] that represents a Storage Trie (MPT) associated with Contract Account.
    Storage(Sha256Hash),
}
...
impl<S: WorldStateStorage + Send + Sync + Clone> KeyspacedInstrumentedDB<S> {
    pub(crate) fn open(storage: S, keyspace: TrieLevel) -> KeyspacedInstrumentedDB<S> {
        KeyspacedInstrumentedDB {
            storage,
            write_set: StorageMutations::new(),
            keyspace,
        }
    }
