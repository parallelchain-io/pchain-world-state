...
//! Functions to initialize/open a Merkle Patricia Trie with given storage soruce, and implementation of the opereation to update, 
...
use crate::{
    keys::{PrefixedTrieNodeKey, WSKey, AppKey}, 
    storage::{WorldStateStorage, KeyspacedInstrumentedDB, StorageMutations}, 
...
impl<S: WorldStateStorage + Send + Sync + Clone> Mpt<S>{
    ...
    pub(crate) fn get_all_elements(&self) -> HashMap<AppKey, Value>{
        let trie = TrieDB::<NoExtensionLayout>::new(self, &self.state_hash).expect("Fail to open worldstate trie");
    ...
    pub(crate) fn get_key_with_proof(&self, key: &WSKey) -> (Proof, Option<Value>){
        ...
        let proof = generate_proof::<_, NoExtensionLayout, _, _>(&trie, vec![key].iter()).expect("Fail to open worldstate trie");
        let value = trie.get(key.as_ref()).unwrap();
    ...
    pub(crate) fn set_value(&mut self, key: WSKey, value: Value){
        ...
        let new_root = {
            ...
            let mut trie = TrieDBMut::<NoExtensionLayout>::from_existing(self, &mut root_hash).expect("Fail to open worldstate trie");
            trie.insert(key.as_ref(), &value).expect("fail to insert to trie.");
...
impl<S: WorldStateStorage + Send + Sync + Clone> HashDB<RefHasher, Value> for Mpt<S>{
    ...
    fn contains(&self, key: &Hash256, mpt_prefix: Prefix) -> bool {
        let key: [u8; 32] = PrefixedTrieNodeKey::<RefHasher>::key(key, mpt_prefix).try_into().unwrap();
